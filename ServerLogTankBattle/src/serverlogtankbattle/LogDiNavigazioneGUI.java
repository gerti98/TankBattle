 ...
 * @author Gerti
 */
public class LogDiNavigazioneGUI implements Serializable{
    ...
    public String serializzaConUTF(){
        ...
        xstream.alias("EventoDiNavigazioneGUI", LogDiNavigazioneGUI.class);    
        return "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" + xstream.toXML(this);
    }
    
    
    private static void valida(String eventXML){ // (00) 
        try{
    ...
    public static LogDiNavigazioneGUI ricevi(ServerSocket serverSocket){ // (01) 
        try (
            ...
            valida(eventXML);
            XStream xstream = new XStream();
            xstream.useAttributeFor(DataFormato.class, "formato"); // (02) 
            xstream.useAttributeFor(OraFormato.class, "formato");
            xstream.alias("EventoDiNavigazioneGUI", LogDiNavigazioneGUI.class);
            return (LogDiNavigazioneGUI)xstream.fromXML(eventXML);
...
/* Note:
    ...
    (00) Gestione validazione XML ricevuto da client
    
    (01) Ricezione dell'evento XML
    
    (02) Gestione corretta deserializzazione attributo formato
*/